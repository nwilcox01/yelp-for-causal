...
```{r}
library(readr)
las_vegas <- read_csv("/Users/naomiwilcox/Downloads/yelp-for-causal-master 5/data/las-vegas-yelp.csv")[,-1]
head(las_vegas)
```
...
# the weighted empirical mean outcome for restaurants with >=3.5 stars
# minus the weighted empirical mean for restaurants with <3.5 stars
...
```{r}
IPTW_or <- mean(wt*as.numeric(las_vegas$star_intervention==1)*las_vegas$open_2019) /
  mean(wt*as.numeric(las_vegas$star_intervention==1)) -
  mean(wt*as.numeric(las_vegas$star_intervention==0)*las_vegas$open_2019) / 
  mean(wt*as.numeric(las_vegas$star_intervention==0))
IPTW_or
```

#  Formal assessmment of positivity

Provide some formal assessment of the positivity assumption. Evaluate the distribution of your estimated propensity score $g_n(A = 1|W_i), i = 1, ..., n,$ and corresponding non-stabilized weights (as well as of your stabilized weights if you use stabilized weights to fit an MSM). Consider evaluating sensitivity to different truncation levels for gn. Note that for TMLE, bounding or truncating gn away from 0 is recommended on the basis of both theory and finite sample performance; for IPTW it can help or hurt. Report how for how many observations was $g_n(A|Wi)$ truncated. 
...
ggplot(wt, aes(wt)) + geom_histogram(fill = "red3", col = "black") + theme_minimal() + 
  labs(title = "Distribution of weights")

# truncate large weights ARBITRARILY at 4
wt.trunc <- wt
...
# evaluate the IPTW estimand with the truncated weights (at 4)
IPTW.tr4 <- mean(wt.trunc*as.numeric(las_vegas$star_intervention==1)*las_vegas$open_2019) -
  mean(wt.trunc*as.numeric(las_vegas$star_intervention==0)*las_vegas$open_2019)
IPTW.tr4

# truncated weights (at 5)
wt.trunc<- wt
wt.trunc[ wt.trunc > 5] = 5

# truncated weights at 5
IPTW.tr5 <- mean(wt.trunc*as.numeric(las_vegas$star_intervention==1)*las_vegas$open_2019) -
  mean(wt.trunc*as.numeric(las_vegas$star_intervention==0)*las_vegas$open_2019)
IPTW.tr5

# practical positivity assumption check
# use the table function to examine support in exposure-covariate strata
# example: table(ObsData$W1,ObsData$W2, ObsData$A)
# all four covariates: age, review, chain, american, with EXPOSURE stars
table(las_vegas$age_binary, las_vegas$review_binary, las_vegas$chain, las_vegas$american,
      las_vegas$star_intervention)
# top table corresponds to no exposure (stars < 3.5) A=0, and bottom table corresponds to exposure (stars >= 3.5) A=1
# the rows correspond to W1-4
# There are certain covariate combinations with *little* in the exposure.  
...
cov_int_df <- las_vegas %>% 
  dplyr::select(american, chain, age_binary, review_binary, star_intervention) %>%
  as.data.frame
# estimate qbar_0(A, W)
...
all_above_df <- las_vegas %>%
  dplyr::select(american, chain, age_binary, review_binary,
         star_intervention) %>%
...
```
The estimate is $\hat{\Psi}_{ss}(P_n) =$ 
`r round(Psi_hat_ss, 3)`.
...
ghat_sl <- SuperLearner(Y = las_vegas$star_intervention,
                        X = cov_int_df %>% dplyr::select(-star_intervention),
